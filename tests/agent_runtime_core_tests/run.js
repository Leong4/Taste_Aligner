...
test("object + object: incoming wins on leaf conflict", () => {
...
test("object + object: nested recursive merge", () => {
    const base = {
...
test("arrays: concatenate with string dedup", () => {
...
test("arrays: id-based dedup for objects", () => {
    const base = {
        items: [
            { id: "a", score: 1.0 },
            { id: "b", score: 0.8 },
...
test("incoming wins on type mismatch", () => {
    const base = { count: 5 };
...
test("mergeTrace deep-merges into existing skill trace", () => {
...
test("mergeTraceBundle deep-merges per key", () => {
    const ctx = createExecutionContext({ text: "test" });
...
test("get throws on missing skill", () => {
    const reg = new SkillRegistry();
    assert.throws(
        () => reg.get("nonexistent"),
        /not found/,
...
test("detects duplicate node ID", () => {
    const errors = validateGraph({
        name: "test",
        version: "1.0",
        nodes: [
...
test("orchestrator stops on terminal signal from skill", async () => {
    const reg = new SkillRegistry();
    reg.register({
        name: "step1",
...
test("renaming node IDs does not break orchestrator", async () => {
    const reg = new SkillRegistry();
    let executionOrder = [];
...
if (failed > 0) {
    ...
    console.log("ALL CORE SMOKE TESTS: PASS");
}
