...
import { Skill, SkillResult, ExecutionContext, FetchRecommendationInput } from "../core/types";
import { ToolClient } from "../tools/toolClient";

export function createFetchRecommendationSkill(
    toolClient: ToolClient
): Skill<FetchRecommendationInput, unknown> {
    return {
        ...
        async execute(
            ...
            if (!observation.ok) {
                throw new Error(
                    `[fetch_recommendation] Gateway call failed: ` +
                    `${observation.error?.code ?? "unknown"} — ${observation.error?.message ?? ""}`
                );
            }

            const reco = observation.output as Record<string, unknown>;
            ...
            const recoTrace = reco.decision_trace as Record<string, unknown> | undefined;
            const trace = {
