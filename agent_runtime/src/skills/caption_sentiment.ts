...
import {
    CaptionSentimentDecisionTrace,
    CaptionSentimentInput,
    CaptionSentimentOutput,
    ExecutionContext,
    Skill,
    SkillResult,
} from "../core/types";
...
const CHINESE_SIGNALS: Array<[string, number]> = [
...
function englishSignals(caption: string): SentimentSignal[] {
    const words = caption
        .toLowerCase()
        .replace(/[’']/g, "")
        .match(/[a-z]+/g) ?? [];
    ...
    for (let index = 0; index < words.length; index++) {
        ...
        if (intensified) score *= 1.2;
        signals.push({ term: negated ? `not_${word}` : word, score: clampSigned(score) });
    }
    return signals;
}

function chineseSignals(caption: string): SentimentSignal[] {
    const signals: SentimentSignal[] = [];
...
export function analyzeCaptionSentiment(captionInput: unknown): CaptionSentimentOutput {
    const caption = typeof captionInput === "string" ? captionInput.trim() : "";
    if (!caption) {
        const trace: CaptionSentimentDecisionTrace = {
            rule_id: RULE_ID,
...
export const captionSentimentSkill: Skill<CaptionSentimentInput, CaptionSentimentOutput> = {
    ...
    async execute(
        input: CaptionSentimentInput,
        _context: ExecutionContext,
    ): Promise<SkillResult<CaptionSentimentOutput>> {
