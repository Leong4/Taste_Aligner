...
export function createBuildCardsSkill(toolClient: ToolClient): Skill<BuildCardsInput, unknown> {
    return {
        ...
        async execute(
            ...
            if (!observation.ok) {
                throw new Error(
                    `[build_cards] Gateway call to planner.compose failed: ` +
                    `${observation.error?.code ?? "unknown"} — ${observation.error?.message ?? ""}`
                );
            }

            const plannerResponse = observation.output as Record<string, unknown>;
            ...
            const plannerTrace = plannerResponse.decision_trace as Record<string, unknown> | undefined;
            const trace = plannerTrace?.planner as Record<string, unknown> ?? {
