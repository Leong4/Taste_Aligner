...
import {
    ...
    BuildProfileVectorInput,
    BuildProfileVectorOutput,
    ProfileVectorDecisionTrace,
} from "../core/types";
 ...
 */
function computeWTime(timestamp: string | number | undefined, nowMs: number): number {
    if (timestamp === undefined || timestamp === null) return 1.0;
    const tsMs =
        typeof timestamp === "number"
            ? timestamp
            : Date.parse(String(timestamp));
    if (!Number.isFinite(tsMs) || tsMs <= 0) return 1.0;
 ...
 */
function computeWSent(sentiment: number | undefined): number {
    const s =
        typeof sentiment === "number" && Number.isFinite(sentiment)
            ? sentiment
            : 0;
...
// ---------------------------------------------------------------------------
// Pure deterministic function (exported for unit testing)
// ---------------------------------------------------------------------------
...
export function buildProfileVector(
    ...
    const perMemory: TopKMemory[] = memories.map((m) => {
...
export function createBuildProfileVectorSkill(): Skill<
    BuildProfileVectorInput,
    BuildProfileVectorOutput
> {
    return {
        ...
        inputSchema: {
            description:
                "Compute profile vector from memory.search weighted results (P4 unification)",
            required: [],
        ...
        outputSchema: {
            description:
                "Profile vector (512-dim), anchors, per-memory weights, and decision trace",
            required: [
                "profile_vector",
                "anchors",
        ...
        async execute(
            input: BuildProfileVectorInput,
            context: ExecutionContext,
        ): Promise<SkillResult<BuildProfileVectorOutput>> {
            let nowMs = FIXED_EPOCH_MS;
            let nowSource: ProfileVectorDecisionTrace["now_source"] = "fixed_epoch";
            if (typeof input.now_ts === "number" && Number.isFinite(input.now_ts)) {
                nowMs = input.now_ts;
                nowSource = "input_now_ts";
            } else if (
                typeof context.request_ts === "number" &&
            ...
            const weighted: MemoryWeightedResult[] = Array.isArray(input.weighted_results)
                ? input.weighted_results
                : [];

            // ── Empty input: deterministic fallback ───────────────────────────
            if (weighted.length === 0) {
                const fallbackTrace: ProfileVectorDecisionTrace = {
                    rule_id: RULE_ID,
            ...
            const memories = weighted as MemoryUnit[];
            const result = buildProfileVector(memories, nowMs);

            const pvTrace: ProfileVectorDecisionTrace = {
                rule_id: RULE_ID,
                schema_version: SCHEMA_VERSION,
                now_source: nowSource,
