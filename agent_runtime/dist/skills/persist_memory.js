...
exports.createPersistMemorySkill = createPersistMemorySkill;
...
async function postMemory(body, timeoutMs) {
    const baseUrl = (process.env.MEMORY_SERVICE_URL ?? "http://localhost:5001").replace(/\/$/, "");
    const url = new node_url_1.URL(`${baseUrl}/write`);
        ...
        try {
            const request = transport.request({
                ...
                response.on("end", () => {
                    ...
                    let parsed = null;
                    try {
                        const candidate = text ? JSON.parse(text) : null;
...
function resultWithTrace(trace) {
    const output = {
    ...
    if (trace.memory_id !== undefined)
        output.memory_id = trace.memory_id;
    if (trace.http_status !== undefined)
        output.http_status = trace.http_status;
    if (trace.error_code !== undefined)
        output.error_code = trace.error_code;
    if (trace.error_message !== undefined)
        output.error_message = trace.error_message;
    return { output, trace };
}
function createPersistMemorySkill() {
    return {
        name: "persist_memory",
        inputSchema: {
        ...
        async execute(input, context) {
            ...
            const memoryId = requestedMemoryId || (0, node_crypto_1.randomUUID)();
            if (!isValidTes(input)) {
                return resultWithTrace({
                    rule_id: RULE_ID,
                    schema_version: SCHEMA_VERSION,
            ...
            const originalImage = normalizeDataUrl(input.image_original_base64 ?? input.image_base64, "image/jpeg");
            if (originalImage)
                body.image_base64 = originalImage;
            ...
            const maxAttempts = positiveInt(process.env.MEMORY_WRITE_MAX_ATTEMPTS, DEFAULT_MAX_ATTEMPTS, 5);
            const retryBaseMs = positiveInt(process.env.MEMORY_WRITE_RETRY_BASE_MS, DEFAULT_RETRY_BASE_MS, 5000);
            let last = {
                ok: false,
                retryable: false,
            ...
            if (last.ok) {
                ...
                if (last.httpStatus !== undefined)
                    trace.http_status = last.httpStatus;
                if (last.idempotentReplay !== undefined)
                    trace.idempotent_replay = last.idempotentReplay;
...
//# sourceMappingURL=persist_memory.js.map