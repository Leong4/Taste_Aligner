...
// Fallback builder
// ---------------------------------------------------------------------------
function buildInputSummary(city, tagsCount, topK, nowTsPresent) {
    const summary = {
...
function buildFallback(reason, tagsCount, topK, userId, city, nowTsPresent, latencyMs, errorMessage) {
    const trace = {
        rule_id: RULE_ID,
...
function createMemoryWeightAdjustSkill(toolClient) {
    return {
        ...
        async execute(input, context) {
            ...
            if (city !== undefined)
                payload.city = city;
            if (rawNowTs !== undefined)
                payload.now_ts = rawNowTs;
            // Call tool
            ...
            if (!observation.ok) {
                return buildFallback("tool_error", tags.length, topK, userId, city, nowTsPresent, latencyMs, observation.error?.message ?? "gateway_call_failed");
            }
